...
import { Activity, Bot, Clock, DollarSign, GitBranch, Shield, TrendingUp, Zap } from "lucide-react";
...
const QUICK_ACTIONS = [
  { label: "Monitoring", desc: "Live orchestration view", icon: Activity, href: "/monitoring", accent: "from-violet-600/20 to-violet-800/10 border-violet-700/30" },
  { label: "Memory", desc: "Semantic memory store", icon: Zap, href: "/memory", accent: "from-cyan-600/20 to-cyan-800/10 border-cyan-700/30" },
  { label: "Workflows", desc: "Autonomous pipelines", icon: GitBranch, href: "/workflows", accent: "from-blue-600/20 to-blue-800/10 border-blue-700/30" },
  { label: "Governance", desc: "Risk & approvals", icon: Shield, href: "/governance", accent: "from-amber-600/20 to-amber-800/10 border-amber-700/30" },
];
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Enterprise AI orchestration — realtime view
          </p>
        </div>
      ...
      >
        ...
        <div className="rounded-xl border border-white/10 bg-zinc-950 divide-y divide-white/5">
          {events.slice(0, 8).map((ev) => (
            <div key={ev.id} className="flex items-center gap-4 px-5 py-3">
              ...
              <span className="text-xs text-zinc-400 font-mono shrink-0">{ev.type}</span>
              <span className="text-xs text-zinc-600 truncate flex-1">
                {Object.entries(ev.payload).slice(0, 2).map(([k, v]) => `${k}=${String(v).slice(0, 30)}`).join("  ")}
              </span>
            </div>
          ))}
          {events.length === 0 && (
