...
const MEMORY_TYPE_CONFIG: Record<MemoryType, { color: string; dot: string }> = {
  semantic: {
    color: "text-violet-300 bg-violet-950/40 border-violet-800/30",
    dot: "bg-violet-400",
  },
...
const MOCK_MEMORIES = [
  {
    ...
    content:
      "The supervisor graph uses a researcher → executor → critic loop with conditional re-routing until the critic passes or max_iterations is reached.",
  ...
  },
  {
    ...
    content:
      "User requested generation of an enterprise monorepo architecture for a luxury multi-agent AI OS. Completed with 85 files.",
  ...
  {
    id: "3",
    memory_type: "user" as MemoryType,
    content:
      "User prefers enterprise-grade, no placeholder code, strict TypeScript, ruff + mypy for Python.",
  ...
  {
    id: "4",
    memory_type: "strategic" as MemoryType,
    content:
      "Goal: build a production-grade luxury AI operating system with realtime monitoring, persistent memory, and governance.",
      ...
      <div className="flex items-center gap-3">
        <div className="relative max-w-sm flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-white/5 py-2 pl-9 pr-3 text-sm text-zinc-300 placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-white/20"
          />
        </div>
        <div className="flex items-center gap-1 rounded-lg border border-white/10 p-1">
          {(
            ["all", "semantic", "episodic", "workflow", "user", "project", "strategic"] as const
          ).map((t) => (
            <button
              key={t}
              onClick={() => {
        ...
        {memories.map((mem, i) => {
          ...
          return (
            <motion.div
              ...
              className="group rounded-xl border border-white/10 bg-zinc-950 p-4 transition-colors hover:border-white/20"
            >
              <div className="mb-3 flex items-start justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button className="text-zinc-700 opacity-0 transition-opacity hover:text-rose-400 group-hover:opacity-100">
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                </div>
              </div>

              <p className="line-clamp-3 text-sm leading-relaxed text-zinc-300">{mem.content}</p>
